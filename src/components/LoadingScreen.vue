...
<template>
  <div class="text-center">
    <v-overlay :model-value="loading" persistent fullscreen full-width class="align-center justify-center" scrim="white">
      <v-container fill-height>
        <v-layout row justify-center align-center>
          <v-progress-circular v-if="showLoader" indeterminate size="70" width="7" color="#28673c">
          </v-progress-circular>
          <v-sheet v-if="showError" class="g-errorSheet">
            <v-card class="mx-auto" width="400" prepend-icon="mdi-close-octagon-outline" variant="tonal" color="error"
              title="ERROR">
              <v-card-text>
                {{ errorMessage }}
              </v-card-text>
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn :append-icon="expand ? 'mdi-chevron-up' : 'mdi-chevron-down'" @click="expand = !expand"
